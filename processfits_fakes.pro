...
PRO processfits_fakes, inputfolder, inputfiles, outputRAW, outputtable
...
new_entry = {BULGE_SERS_N:dblarr(nfits,nbands), $
             ...
             DOF:dblarr(nfits,nbands), $
             SKY_COUNTS:dblarr(5), GAIN:dblarr(5)}

table = replicate(new_entry, n_elements(data) )

fb = 1
fitbands = 0
table.CHI_SQ[*,0,*] = [reform(data.CHISQ_BULGE,1,fb,n_elements(data)),$
...
for i=0L, n_elements(data)-1 do begin
    ...
    if not keyword_set(no_colors) then begin
        ...
        my_color = where(bands eq fiducial)
